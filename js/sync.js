...
class AuthStore{
  ...
  async init(){
    ...
    if(firebaseConfigured()){
      ...
      }catch(e){console.error('Firebase init failed; using local fallback',e)}
    }
...
export class SyncStore{
  constructor(){this.mode='local';this.listeners=[];this.statusListeners=[];this.data=null;this.userId=null;this.unsubscribe=null;this.cloudRef=null;this.saveTimer=null;this.applyingRemote=false;this.status={mode:'local',state:'idle',message:'Локален режим',lastSync:null};}
  ...
  async _startCloud(u){
    ...
    try{
      ...
      this.unsubscribe=f.onSnapshot(this.cloudRef,{includeMetadataChanges:true},snap=>{
        ...
        const remote=snap.data();const cloudData=remote?.payload||remote;if(!cloudData)return;
  ...
  async _pushCloud(force=false){if(this.mode!=='firestore'||!this.cloudRef||!this.data)return false;try{const f=auth.fb.firestoreMod;await f.setDoc(this.cloudRef,{schemaVersion:1,updatedAt:f.serverTimestamp(),updatedBy:'web',payload:this.snapshot()},{merge:false});this._setStatus({state:'ok',message:'Синхронизирано',lastSync:new Date().toISOString()});return true}catch(e){console.error(e);this._setStatus({state:'error',message:humanizeFirebaseError(e).message});if(force)throw e;return false}}
