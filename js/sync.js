...
export class SyncStore{
  ...
  async _pushCloud(force=false){if(this.mode!=='firestore'||!this.cloudRef||!this.data)return false;try{const f=auth.fb.firestoreMod;await f.setDoc(this.cloudRef,{schemaVersion:2,updatedAt:f.serverTimestamp(),updatedBy:'web',payload:this.snapshot()},{merge:true});this._setStatus({state:'ok',message:'Синхронизирано',lastSync:new Date().toISOString()});return true}catch(e){console.error(e);this._setStatus({state:'error',message:humanizeFirebaseError(e).message});if(force)throw e;return false}}
