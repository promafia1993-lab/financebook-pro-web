...
export function repairCreditCardData(data){
  ...
  for(const account of data.accounts){
    ...
    const amount=Math.max(0,Math.abs(Number(account.balance||0)||0));
    // FinanceBook uses a positive Web balance for credit-card outstanding debt.
    // Normalize payloads written by older builds so both clients use one sign.
    if(Number(account.balance||0)!==amount){account.balance=amount;changed=true}
    if(!debt){
  ...
  _repairCreditCardBundles(base){
    ...
    for(const a of base.accounts){
      ...
      if(!d){
        ...
        d={id,name:a.name||'Кредитна карта',institution:a.bank||'',amount:Math.max(0,Math.abs(num(a.balance))),original:Math.max(0,Math.abs(num(a.balance))),monthly:num(a.minimumPayment),apr:num(a.creditInterestRate),dueDay:num(a.paymentDueDay)||15,due:'',paid:0,archived:a.active===false,linkedAccountId:a.id,debtType:'creditCard',note:a.note||''};
        ...
        if(num(d.amount)<=0&&Math.abs(num(a.balance))>0)d.amount=Math.abs(num(a.balance));
        if(num(d.original)<=0)d.original=Math.max(num(d.amount),Math.abs(num(a.balance)));
      }
      a.linkedDebtId=d.id;
    }
  }
