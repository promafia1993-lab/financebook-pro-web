import {store,auth} from './sync-v220.js?v=220';
...
const schemas={
 accounts:{title:'Сметки и портфейли',add:'Сметка',fields:[
  ['name','Име','text'],['bank','Банка / институция','select',optionString(BULGARIAN_BANKS)],['balance','Текущ баланс','number'],
...
function effectiveDebts(){
 ...
 for(const a of (data.accounts||[]).filter(x=>x.type==='creditCard')){
   ...
   const amount=Math.max(0,Math.abs(Number(a.balance||0)||0));
   if(!d){
...
function debtCardHtml(x){
 ...
 if(isCard){
   const limit=Number(account?.creditLimit||0),available=Math.max(0,limit-amount),util=limit>0?Math.min(100,amount/limit*100):0;
...
function debtsPage(){
 ...
 const loansRemaining=loans.reduce((s,x)=>s+Number(x.amount||0),0),cardRemaining=cards.reduce((s,x)=>s+Number(x.amount||0),0),monthly=active.reduce((s,x)=>s+Number(x.monthly||0),0),paidMonth=(data.debtPayments||[]).filter(p=>{const d=new Date(p.paidAt);const n=new Date();return !Number.isNaN(d.getTime())&&d.getFullYear()===n.getFullYear()&&d.getMonth()===n.getMonth()}).reduce((s,p)=>s+Number(p.amount||0),0);
...
function recordDebtPayment(id){
 ...
 if(account){
   store.update('accounts',account.id,{balance:remaining});
...
function statisticsPage(){
 const income=data.transactions.filter(x=>x.amount>0).reduce((s,x)=>s+Number(x.amount),0),expense=-data.transactions.filter(x=>x.amount<0).reduce((s,x)=>s+Number(x.amount),0),net=data.accounts.reduce((s,x)=>s+Number(x.balance),0),debt=effectiveDebts().reduce((s,x)=>s+Number(x.amount),0);
