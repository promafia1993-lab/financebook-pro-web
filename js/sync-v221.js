...
export function repairCreditCardData(data){
  if(!data||!Array.isArray(data.accounts))return false;
  if(!Array.isArray(data.debts))data.debts=[];
  let changed=false;
  const canonicalByAccount=new Map();
  const nextDueForDay=(rawDay)=>{
    const day=Math.max(1,Math.min(31,Number(rawDay||15)||15));
    const now=new Date();
    const make=(year,month)=>{
      const last=new Date(year,month+1,0).getDate();
      const d=new Date(year,month,Math.min(day,last),12,0,0,0);
      return d;
    };
    let due=make(now.getFullYear(),now.getMonth());
    const today=new Date(now.getFullYear(),now.getMonth(),now.getDate(),12,0,0,0);
    if(due<today)due=make(now.getFullYear(),now.getMonth()+1);
    return `${due.getFullYear()}-${String(due.getMonth()+1).padStart(2,'0')}-${String(due.getDate()).padStart(2,'0')}`;
  };
  for(const account of data.accounts){
    const type=String(account.type||'bank');
    const name=normName(account.name);
    const hasCreditMetadata=Number(account.creditLimit||0)>0 || Number(account.minimumPayment||0)>0 || Number(account.creditInterestRate||0)>0;
    const nameLooksLikeCard=name.includes('кредитн') || name.includes('credit card') || name.includes('shopping limit') || name.includes('шопинг лимит');
    const convertible=['bank','card','other',''].includes(type);
    if(type!=='creditCard' && convertible && (hasCreditMetadata||nameLooksLikeCard)){
      account.type='creditCard';
      changed=true;
    }
    if(account.type!=='creditCard')continue;
    if(account.netWorth!==false){account.netWorth=false;changed=true}
    if(!Number(account.paymentDueDay||0)){account.paymentDueDay=15;changed=true}
    if(!Number(account.statementDay||0)){account.statementDay=1;changed=true}

    const accountId=String(account.id??'');
    let debt=data.debts.find(d=>String(d.linkedAccountId??'')===accountId || (account.linkedDebtId!=null && String(d.id??'')===String(account.linkedDebtId)));
    if(!debt){
      debt=data.debts.find(d=>String(d.debtType||'')==='creditCard' && normName(d.name)===name && (!d.linkedAccountId || String(d.institution||'')===String(account.bank||'')));
    }
    const limit=Math.max(0,Number(account.creditLimit||0)||0);
    const available=Number(account.balance||0)||0;
    const amount=Math.max(0,limit-available);
    // Canonical v3 semantics: for a credit card, account.balance is the
    // available/free credit. Outstanding debt is always derived as
    // creditLimit - availableCredit.
    if(account.creditBalanceMode!=='available'){account.creditBalanceMode='available';changed=true}
    if(!debt){
      const stableId=account.id!=null?`card-debt-${account.id}`:crypto.randomUUID();
      debt={id:stableId,name:account.name||'Кредитна карта',institution:account.bank||'',amount,original:amount,paid:0,paidAmount:0,monthly:Number(account.minimumPayment||0),apr:Number(account.creditInterestRate||0),dueDay:Number(account.paymentDueDay||15),due:nextDueForDay(account.paymentDueDay),debtType:'creditCard',linkedAccountId:account.id,archived:account.active===false,note:account.note||'',updatedAt:new Date().toISOString()};
      data.debts.unshift(debt);
      changed=true;
    }else{
      const dueValue=String(debt.due||'').trim()||nextDueForDay(account.paymentDueDay);
      const patch={name:account.name||debt.name,institution:account.bank||debt.institution||'',amount,original:amount,paid:0,paidAmount:0,monthly:Number(account.minimumPayment||0),apr:Number(account.creditInterestRate||0),dueDay:Number(account.paymentDueDay||15),due:dueValue,debtType:'creditCard',linkedAccountId:account.id,archived:account.active===false,note:account.note||debt.note||''};
      for(const [k,v] of Object.entries(patch)){
        if(String(debt[k]??'')!==String(v??'')){debt[k]=v;changed=true}
      }
    }
    canonicalByAccount.set(accountId,String(debt.id??''));
    if(String(account.linkedDebtId??'')!==String(debt.id??'')){
      account.linkedDebtId=debt.id;
      changed=true;
    }
  }

  // Remove duplicate companion card-debt rows left by older repairs. Keep
  // standalone credit-card debts that are not linked to a real account.
  const seenLinks=new Set();
  const before=data.debts.length;
  data.debts=data.debts.filter(d=>{
    if(String(d.debtType||'')!=='creditCard' || d.linkedAccountId==null)return true;
    const key=String(d.linkedAccountId);
    const canonical=canonicalByAccount.get(key);
    if(canonical && String(d.id)!==canonical)return false;
    if(seenLinks.has(key))return false;
    seenLinks.add(key);return true;
  });
  if(data.debts.length!==before)changed=true;
  return changed;
}
...
export class SyncStore{
  ...
  async loadForUser(u){
    ...
    this.data=this.mergeDefaults(obj);repairCreditCardData(this.data);this._applyUserProfile(u);this._saveLocal(false);
  ...
  _normalizeRemotePayload(remote){
    const cloudData=clone(remote?.payload||remote||{});
    // Android schema v2 published credit-card balance as OUTSTANDING DEBT.
    // Convert only that known legacy Android payload. Old Web payloads used
    // the field as the available amount shown in the editor.
    if(remote?.updatedBy==='android'&&Number(remote?.schemaVersion||0)<3&&Array.isArray(cloudData.accounts)){
      for(const a of cloudData.accounts){
        if(String(a?.type||'')!=='creditCard')continue;
        const limit=Number(a.creditLimit||0)||0,oldValue=Math.abs(Number(a.balance||0)||0),minimum=Number(a.minimumPayment||0)||0;
        const looksLikeWebAvailable=minimum>0&&oldValue<minimum;
        a.balance=looksLikeWebAvailable?oldValue:limit-oldValue;a.creditBalanceMode='available';a.netWorth=false;
      }
    }
    return cloudData;
  }
  async _startCloud(u){
    ...
    try{
      ...
      if(snap.exists()){
        const remote=snap.data();const cloudData=this._normalizeRemotePayload(remote);
        this.applyingRemote=true;this.data=this.mergeDefaults(cloudData);const repaired=repairCreditCardData(this.data);this._applyUserProfile(u);this._saveLocal(false);this.applyingRemote=false;if(repaired)await this._pushCloud(true);
      }else{
        await this._pushCloud(true);
      }
      this.unsubscribe=f.onSnapshot(this.cloudRef,{includeMetadataChanges:true},snap=>{
        if(!snap.exists()||snap.metadata.hasPendingWrites)return;
        const remote=snap.data();if(remote?.updatedBy==='web'&&remote?.updatedDevice===this.deviceId)return;const cloudData=this._normalizeRemotePayload(remote);if(!cloudData)return;
        this.applyingRemote=true;this.data=this.mergeDefaults(cloudData);const repaired=repairCreditCardData(this.data);this._applyUserProfile(auth.currentUser()||u);this._saveLocal(false);this.applyingRemote=false;this.listeners.forEach(fn=>fn(this.snapshot()));if(repaired)this._queueCloud();
  ...
  mergeDefaults(saved){const base=emptyData(saved?.profile||{});for(const [k,v] of Object.entries(saved||{})){if(Array.isArray(v))base[k]=v;else if(v&&typeof v==='object')base[k]={...(base[k]||{}),...v};else base[k]=v}this._repairCreditCardBundles(base);return base}
  _repairCreditCardBundles(base){
    if(!base||!Array.isArray(base.accounts))return;
    if(!Array.isArray(base.debts))base.debts=[];
    const num=v=>Number(v||0)||0;
    for(const a of base.accounts){
      const name=String(a?.name||'').toLowerCase();
      const hasMeta=num(a?.creditLimit)>0||num(a?.minimumPayment)>0||num(a?.creditInterestRate)>0;
      const looks=name.includes('кредитн')||name.includes('credit card')||name.includes('shopping limit')||name.includes('шопинг лимит');
      if((hasMeta||looks)&&['bank','card','other','creditCard',undefined,null,''].includes(a.type))a.type='creditCard';
      if(a.type!=='creditCard')continue;
      a.creditBalanceMode='available';a.netWorth=false;
      let d=base.debts.find(x=>String(x?.linkedAccountId??'')===String(a.id??'')||String(a.linkedDebtId??'')===String(x?.id??''));
      if(!d){
        const id=a.linkedDebtId||`card-debt-${a.id||crypto.randomUUID()}`;
        const cardDebt=Math.max(0,num(a.creditLimit)-num(a.balance));
        d={id,name:a.name||'Кредитна карта',institution:a.bank||'',amount:cardDebt,original:cardDebt,monthly:num(a.minimumPayment),apr:num(a.creditInterestRate),dueDay:num(a.paymentDueDay)||15,due:'',paid:0,archived:a.active===false,linkedAccountId:a.id,debtType:'creditCard',note:a.note||''};
        base.debts.unshift(d);
      }else{
        d.name=a.name||d.name;d.institution=a.bank||d.institution||'';d.monthly=num(a.minimumPayment)||num(d.monthly);d.apr=num(a.creditInterestRate)||num(d.apr);d.dueDay=num(a.paymentDueDay)||num(d.dueDay)||15;d.linkedAccountId=a.id;d.debtType='creditCard';d.archived=a.active===false;
        const cardDebt=Math.max(0,num(a.creditLimit)-num(a.balance));
        d.amount=cardDebt;d.original=cardDebt;d.paid=0;
      }
      a.linkedDebtId=d.id;
    }
  }
  ...
  save(notify=true){if(this.data)repairCreditCardData(this.data);this._saveLocal(notify);if(this.mode==='firestore'&&!this.applyingRemote)this._queueCloud()}
  ...
  async _pushCloud(force=false){if(this.mode!=='firestore'||!this.cloudRef||!this.data)return false;try{const f=auth.fb.firestoreMod;await f.setDoc(this.cloudRef,{schemaVersion:3,updatedAt:f.serverTimestamp(),updatedBy:'web',updatedDevice:this.deviceId,payload:this.snapshot()},{merge:true});this._setStatus({state:'ok',message:'Синхронизирано',lastSync:new Date().toISOString()});return true}catch(e){console.error(e);this._setStatus({state:'error',message:humanizeFirebaseError(e).message});if(force)throw e;return false}}
  ...
  update(collection,id,patch){const x=(this.data[collection]||[]).find(v=>String(v.id)===String(id));if(x){Object.assign(x,patch,{updatedAt:new Date().toISOString()});this.save();return x}}
  remove(collection,id){if(collection==='accounts'){const account=(this.data.accounts||[]).find(v=>String(v.id)===String(id));if(account?.type==='creditCard'){this.data.debts=(this.data.debts||[]).filter(d=>String(d.linkedAccountId??'')!==String(id)&&String(d.id??'')!==String(account.linkedDebtId??''));}}this.data[collection]=(this.data[collection]||[]).filter(v=>String(v.id)!==String(id));this.save()}
  setObject(key,patch){this.data[key]={...(this.data[key]||{}),...patch};this.save()}
  replaceAll(next){this.data=this.mergeDefaults(next);repairCreditCardData(this.data);const u=auth.currentUser();if(u)this._applyUserProfile(u);this.save()}
