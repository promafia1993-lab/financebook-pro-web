...
store.init().then(d=>{data=d;const hint=qs('#authModeHint');if(hint)hint.textContent=auth.isFirebase()?'Firebase Authentication и Cloud Firestore са готови за един профил на сайта и приложението.':'Локален режим. Добави Web Firebase config в js/firebase-config.js.';store.onChange(d=>{data=d;if(d)render()});if(auth.currentUser()){nav();render();hideAuth()}else showAuth('login')}).catch(e=>{console.error(e);showAuth('login');qs('#authError').textContent='Грешка при стартиране: '+e.message});
