...
store.init().then(d=>{data=d;const hint=qs('#authModeHint');if(hint)hint.textContent=auth.isFirebase()?'Firebase Authentication е активно. Един и същ профил може да се използва на сайта и в приложението.':'Локален режим. Добави Web Firebase config в js/firebase-config.js, за да активираш истинската регистрация.';const st=qs('#syncText');if(st)st.textContent=auth.isFirebase()?'Firebase Auth активно':'Локален режим';store.onChange(d=>{data=d;if(d)render()});if(auth.currentUser()){nav();render();hideAuth()}else showAuth('login')}).catch(e=>{console.error(e);showAuth('login');qs('#authError').textContent='Грешка при стартиране: '+e.message});
